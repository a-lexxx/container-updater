...
class DockerManager extends Docker {
    ...
    _getServicedList(repoName) {
        ...
        return this.listContainers(this._options.filter)
            .then(workingCts =>
                Promise.all(workingCts.map(ct => this.getImage(ct.ImageID).inspect()
                    .then(imageInfo =>
                        ct.imageInfos = [ct.Image].concat(imageInfo.RepoDigests.map(str => str.split('@')[0])))
                )).then(() => workingCts)
            )
            .then(workingCts => {
                const filteredList = workingCts
                    .filter(container => container.imageInfos.some(info => info === repoName))
    ...
    update(updaterConfig, repoName, imageTag) {
        ...
        return Promise.resolve(
            ...
            .then(() => this._getServicedList(repoName))
            .then(ctsList => {
